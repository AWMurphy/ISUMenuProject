...
def refresh_DiningHalls():
    ...
    for place, url in links.items():
        ...
        try:
            response = requests.get(url + "2026-05-31", headers=headers) # use the headers above along with the url from the dining halls json / today.strftime("%Y-%m-%d")
